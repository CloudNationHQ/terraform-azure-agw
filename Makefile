...
test:
	cd tests && go test -v -timeout 120m -run '^TestApplyNoError$$' -args $(TEST_ARGS) .

test-sequential:
	cd tests && go test -v -timeout 120m -run '^TestApplyAllSequential$$' -args $(TEST_ARGS) .

test-parallel:
	cd tests && go test -v -timeout 120m -run '^TestApplyAllParallel$$' -args $(TEST_ARGS) .

test-local:
	cd tests && go test -v -timeout 120m -run '^TestApplyAllLocal$$' -args $(TEST_ARGS) .
